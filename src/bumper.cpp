...
*/
  ...
  */
  ...
  End(); // Calling end of sensor message
}

void GazeboRosBumper::End()
{
    ros::Publisher pub = rosnode_->advertise<std_msgs::Bool>("test", 1); // Publish if test is done
    ros::Time begin = ros::Time::now() + ros::Duration(10);
    while (ros::ok() && ros::Time::now() < begin)
    {
        std_msgs::Bool msg;
        msg.data = true;
        pub.publish(msg);
